                ...
                for (List<FullDesk> component : connectedComponents) {
                    for (FullDesk fullDesk : component) {
                        fullDesk.getG_desk().setOnClickListener(fullDesk.getDesk().isReserved() ? null : v -> {
                            if (!Database.getInstance(ContextHelper.retrieveContext(), Constants._MAIN_DATABASE).billInterface().getWithLexinTableID(fullDesk.getDesk().getLexinTableID(), Helper.getRestaurantSelectionQRCode()).isEmpty()) {
                                Database.getInstance(ContextHelper.retrieveContext(), Constants._MAIN_DATABASE).billInterface().deleteWithLexinTableID(fullDesk.getDesk().getLexinTableID(), Helper.getRestaurantSelectionQRCode());
